...
SELECT film_id,title,description,release_year,rental_rate FROM film WHERE title='horsnormes';
-- No luck finding your movie? Maybe you made a mistake spelling the name. Write a query to get the film ID, title, description, length and the rental rate of all the movies starting with the two first letters of your favorite movie.
SELECT film_id,title,description,release_year,rental_rate FROM film WHERE  title ILIKE 'ho%';
...
SELECT last_name , amount , payment_date FROM customer INNER JOIN payment ON customer.customer_id = payment.customer_id ORDER BY payment.customer_id;
-- You need to check your inventory. Write a query to get all the movies which are not in inventory.
SELECT title,inventory_id FROM film INNER JOIN inventory ON film.film_id=inventory.film_id WHERE film.film_id != inventory.film_id ;
